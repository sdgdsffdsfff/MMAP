...
BEGIN
	...
	--查询ST层表中是否有'当日'数据
	SELECT COUNT(1) INTO ST_ROW FROM MMAPST.ST_PRODUCT WHERE PERIOD_ID=DM_TODAY;
	IF ST_ROW>0
	THEN
		IO_ROW:=0;
		--备份"上日"数据
		...
		SELECT COUNT(1) INTO ST_ROW FROM MMAPDM.DM_PRODUCT WHERE PERIOD_ID=DM_YESTERDAY;
		IF ST_ROW>0
		THEN
		...
		END IF;
		--本调度逻辑为全删全插
		DM_SQL :='DELETE FROM MMAPDM.DM_PRODUCT' ;	 -- 删除表中数据
		EXECUTE IMMEDIATE DM_SQL;
