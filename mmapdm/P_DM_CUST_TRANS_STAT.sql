CREATE OR REPLACE PROCEDURE "P_DM_CUST_TRANS_STAT"
...
--如果DM层有"当日"数据，则进行数据抽取，否则保持DM层现有数据不变。
  ...
  COMMIT;
      ...
      --调度周频度数据
    P_DM_CUST_TRANS_STAT_FREQ('DM_CUST_TRANS_W_STAT','W','WEEKOFYEAR','DAYOFWEEK',7,DM_TODAY,DM_YESTERDAY,IO_ROW,IO_STATUS_PREQ);
    IO_ROW_ALL := IO_ROW;
    IO_STATUS:=IO_STATUS_PREQ;
   --调度月频度数据
    P_DM_CUST_TRANS_STAT_FREQ('DM_CUST_TRANS_M_STAT','M','MONTH','DAYOFMONTH',6,DM_TODAY,DM_YESTERDAY,IO_ROW,IO_STATUS_PREQ);
    IO_ROW_ALL := IO_ROW_ALL+IO_ROW;
    IO_STATUS:=IO_STATUS+IO_STATUS_PREQ;
    --调度季频度数据
    P_DM_CUST_TRANS_STAT_FREQ('DM_CUST_TRANS_Q_STAT','Q','QUARTER','DAYOFQUARTER',3,DM_TODAY,DM_YESTERDAY,IO_ROW,IO_STATUS_PREQ);
    IO_ROW_ALL := IO_ROW_ALL+IO_ROW;
    IO_STATUS:=IO_STATUS+IO_STATUS_PREQ;
    --调度年频度数据
    P_DM_CUST_TRANS_STAT_FREQ('DM_CUST_TRANS_Y_STAT','Y','YEAR','DAYOFYEAR',2,DM_TODAY,DM_YESTERDAY,IO_ROW,IO_STATUS_PREQ);
    IO_ROW_ALL := IO_ROW_ALL+IO_ROW;
    IO_STATUS:=IO_STATUS+IO_STATUS_PREQ;
