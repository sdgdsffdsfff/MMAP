...
FROM USER_TABLES 
WHERE TABLE_NAME = 'TMP_DM_CUST_TRANS_STAT'
;
IF COUNT_NUM >0
THEN 
  DM_SQL := 'DROP TABLE MMAPDM.TMP_DM_CUST_TRANS_STAT PURGE';
...
DM_SQL :=
  ...
  */
  ...
  ,d.CUST_TRANS_CNT_LC   AS CUST_TRANS_CNT_AVG_SQT_LC 
    ...
    ------FOR MAX/MIN-------------------
  FROM MMAPDM.TMP_DM_CUST_TRANS_STAT A
  FULL JOIN MMAPDM.DM_CUST_TRANS_STAT B
  ON A.CUSTOMER_ID=B.CUSTOMER_ID
  AND A.TRANSCDE_ID=B.TransTyp_ID 
  AND A.CHN_ID=B.CHANNEL
  AND A.TransBr_ID=B.TRANSBR_ID
  AND A.PRODTYP_ID=B.PROD_TYPE
  AND A.YEAR=B.YEAR
  AND B.FREQ=''W''
  AND A.WEEKOFYEAR=B.FREQ_VALUE
  
  --FOR corresponding period---
  FULL JOIN MMAPDM.DM_CUST_TRANS_STAT C
  ON A.CUSTOMER_ID=C.CUSTOMER_ID
  AND A.TRANSCDE_ID=C.TransTyp_ID 
  AND A.CHN_ID=C.CHANNEL
  AND A.TransBr_ID=C.TRANSBR_ID
  AND A.PRODTYP_ID=C.PROD_TYPE
  AND A.YEAR - 1 = c.YEAR
  AND C.FREQ=''W''
  AND A.WEEKOFYEAR=C.FREQ_VALUE
  
  --FOR prior period---
  FULL JOIN MMAPDM.DM_CUST_TRANS_STAT D
  ON A.CUSTOMER_ID=D.CUSTOMER_ID
  AND A.TRANSCDE_ID=D.TransTyp_ID 
  AND A.CHN_ID=D.CHANNEL
  AND A.TransBr_ID=D.TRANSBR_ID
  AND A.PRODTYP_ID=D.PROD_TYPE
  AND A.YEAR=D.YEAR
  AND D.FREQ=''W''
  AND A.WEEKOFYEAR - 1 = D.FREQ_VALUE
'
;
EXECUTE IMMEDIATE DM_SQL;
IO_ROW := SQL%ROWCOUNT ;
...
DM_SQL :=
  ...
  */
  ...
  --THE MAX VALUE FOR RMB TRADE AMOUNT 
  ,(CASE WHEN A.DAYOFMONTH = ''1'' THEN A.CUST_TRANS_AMT_LC  
  ELSE         
  greatest(NVL(A.CUST_TRANS_AMT_LC,0),NVL(B.CUST_TRANS_AMT_LC,0)) END)   
  AS CUST_TRANS_AMT_MAX_LC
  --THE DATE OF RMB TRADE AMOUNT MAX VALUE
  ,(CASE WHEN A.DAYOFMONTH = ''1'' THEN A.PERIOD_ID    
  ELSE        
  (CASE WHEN NVL(A.CUST_TRANS_AMT_LC,0) - NVL(B.CUST_TRANS_AMT_LC,0) > 0 THEN A.PERIOD_ID ELSE B.PERIOD_ID END) END)     
  AS CUST_TRANS_AMT_MAX_DATE_LC
  --THE MIN VALUE FOR RMB TRADE AMOUNT 
  ...
  */
  ,(CASE WHEN A.DAYOFMONTH = ''1'' THEN A.CUST_TRANS_AMT_LC
  ELSE         
  ((NVL(B.CUST_TRANS_AMT_LC,0) * (A.DAYOFMONTH-1) + NVL(A.CUST_TRANS_AMT_LC,0)) / A.DAYOFMONTH) END)  
  AS CUST_TRANS_AMT_AVG_LC
  --DAILY AVERAGE FOR RMB TRADE IN CORRESPONDING PERIOD     
  ,C.CUST_TRANS_CNT_LC  AS CUST_TRANS_AMT_AVG_CWS_LC
  --DAILY AVERAGE FOR RMB TRADE IN PRIOR PERIOD
  ...
  */
  ...
  --THE MAX VALUE FOR NUMBER OF TRANS FOR RMB TRADE
  ,(CASE WHEN A.DAYOFMONTH = ''1'' THEN A.CUST_TRANS_CNT_LC    
  ELSE         
  greatest(NVL(A.CUST_TRANS_CNT_LC,0),NVL(B.CUST_TRANS_CNT_LC,0)) END)   
  AS CUST_TRANS_CNT_MAX_LC
  --THE DATE OF MAX VALUE
  ...
  */
  ,(CASE WHEN A.DAYOFMONTH = ''1'' THEN A.CUST_TRANS_CNT_LC    
  ELSE         
  ((NVL(B.CUST_TRANS_CNT_LC,0) * (A.DAYOFMONTH-1) + NVL(A.CUST_TRANS_CNT_LC,0)) / A.DAYOFMONTH) END)  
  AS CUST_TRANS_CNT_AVG_LC
  --DAILY AVERAGE FOR NUMBER OF TRANS IN CORRESPONDING PERIOD 
  ,c.CUST_TRANS_CNT_LC   AS CUST_TRANS_CNT_AVG_CWS_LC 
  --DAILY AVERAGE FOR NUMBER OF TRANS IN PRIOR PERIOD
...
/*
  4. delete the expire data(retain data 12 cyc for week-data)
*/
...
DM_SQL :=
  ...
  */
  ...
  --THE MAX VALUE FOR RMB TRADE AMOUNT 
  ,(CASE WHEN A.DAYOFQUARTER = ''1'' THEN A.CUST_TRANS_AMT_LC  
  ELSE         
  greatest(NVL(A.CUST_TRANS_AMT_LC,0),NVL(B.CUST_TRANS_AMT_LC,0)) END)   
  AS CUST_TRANS_AMT_MAX_LC
  --THE DATE OF RMB TRADE AMOUNT MAX VALUE
  ,(CASE WHEN A.DAYOFQUARTER = ''1'' THEN A.PERIOD_ID    
  ELSE        
  (CASE WHEN NVL(A.CUST_TRANS_AMT_LC,0) - NVL(B.CUST_TRANS_AMT_LC,0) > 0 THEN A.PERIOD_ID ELSE B.PERIOD_ID END) END)     
  AS CUST_TRANS_AMT_MAX_DATE_LC
  --THE MIN VALUE FOR RMB TRADE AMOUNT 
  ...
  */
  ,(CASE WHEN A.DAYOFQUARTER = ''1'' THEN A.CUST_TRANS_AMT_LC
  ELSE         
  ((NVL(B.CUST_TRANS_AMT_LC,0) * (A.DAYOFQUARTER-1) + NVL(A.CUST_TRANS_AMT_LC,0)) / A.DAYOFQUARTER) END)  
  AS CUST_TRANS_AMT_AVG_LC
  --DAILY AVERAGE FOR RMB TRADE IN CORRESPONDING PERIOD     
  ,C.CUST_TRANS_CNT_LC  AS CUST_TRANS_AMT_AVG_CWS_LC
  --DAILY AVERAGE FOR RMB TRADE IN PRIOR PERIOD
  ...
  */
  ...
  --THE MAX VALUE FOR NUMBER OF TRANS FOR RMB TRADE
  ,(CASE WHEN A.DAYOFQUARTER = ''1'' THEN A.CUST_TRANS_CNT_LC    
  ELSE         
  greatest(NVL(A.CUST_TRANS_CNT_LC,0),NVL(B.CUST_TRANS_CNT_LC,0)) END)   
  AS CUST_TRANS_CNT_MAX_LC
  --THE DATE OF MAX VALUE
  ...
  */
  ,(CASE WHEN A.DAYOFQUARTER = ''1'' THEN A.CUST_TRANS_CNT_LC    
  ELSE         
  ((NVL(B.CUST_TRANS_CNT_LC,0) * (A.DAYOFQUARTER-1) + NVL(A.CUST_TRANS_CNT_LC,0)) / A.DAYOFQUARTER) END)  
  AS CUST_TRANS_CNT_AVG_LC
  --DAILY AVERAGE FOR NUMBER OF TRANS IN CORRESPONDING PERIOD 
  ,c.CUST_TRANS_CNT_LC   AS CUST_TRANS_CNT_AVG_CWS_LC 
  --DAILY AVERAGE FOR NUMBER OF TRANS IN PRIOR PERIOD
...
DM_SQL :=
  ...
  */
  ...
  --THE MAX VALUE FOR RMB TRADE AMOUNT 
  ,(CASE WHEN A.DAYOFYEAR = ''1'' THEN A.CUST_TRANS_AMT_LC  
  ELSE         
  greatest(NVL(A.CUST_TRANS_AMT_LC,0),NVL(B.CUST_TRANS_AMT_LC,0)) END)   
  AS CUST_TRANS_AMT_MAX_LC
  --THE DATE OF RMB TRADE AMOUNT MAX VALUE
  ,(CASE WHEN A.DAYOFYEAR = ''1'' THEN A.PERIOD_ID    
  ELSE        
  (CASE WHEN NVL(A.CUST_TRANS_AMT_LC,0) - NVL(B.CUST_TRANS_AMT_LC,0) > 0 THEN A.PERIOD_ID ELSE B.PERIOD_ID END) END)     
  AS CUST_TRANS_AMT_MAX_DATE_LC
  --THE MIN VALUE FOR RMB TRADE AMOUNT 
  ...
  */
  ,(CASE WHEN A.DAYOFYEAR = ''1'' THEN A.CUST_TRANS_AMT_LC
  ELSE         
  ((NVL(B.CUST_TRANS_AMT_LC,0) * (A.DAYOFYEAR-1) + NVL(A.CUST_TRANS_AMT_LC,0)) / A.DAYOFYEAR) END)  
  AS CUST_TRANS_AMT_AVG_LC
  --DAILY AVERAGE FOR RMB TRADE IN CORRESPONDING PERIOD     
  ,C.CUST_TRANS_CNT_LC  AS CUST_TRANS_AMT_AVG_CWS_LC
  --DAILY AVERAGE FOR RMB TRADE IN PRIOR PERIOD
  ...
  */
  ...
  --THE MAX VALUE FOR NUMBER OF TRANS FOR RMB TRADE
  ,(CASE WHEN A.DAYOFYEAR = ''1'' THEN A.CUST_TRANS_CNT_LC    
  ELSE         
  greatest(NVL(A.CUST_TRANS_CNT_LC,0),NVL(B.CUST_TRANS_CNT_LC,0)) END)   
  AS CUST_TRANS_CNT_MAX_LC
  --THE DATE OF MAX VALUE
  ...
  */
  ,(CASE WHEN A.DAYOFYEAR = ''1'' THEN A.CUST_TRANS_CNT_LC    
  ELSE         
  ((NVL(B.CUST_TRANS_CNT_LC,0) * (A.DAYOFYEAR-1) + NVL(A.CUST_TRANS_CNT_LC,0)) / A.DAYOFYEAR) END)  
  AS CUST_TRANS_CNT_AVG_LC
  --DAILY AVERAGE FOR NUMBER OF TRANS IN CORRESPONDING PERIOD 
  ,c.CUST_TRANS_CNT_LC   AS CUST_TRANS_CNT_AVG_CWS_LC 
  --DAILY AVERAGE FOR NUMBER OF TRANS IN PRIOR PERIOD
