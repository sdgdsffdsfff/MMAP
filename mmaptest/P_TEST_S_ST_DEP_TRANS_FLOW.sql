CREATE OR REPLACE PROCEDURE "MMAPTEST"."P_TEST_S_ST_DEP_TRANS_FLOW"

AS
/* 定义变量*/
TRANS_NAME              VARCHAR2(50) :='P_TEST_S_ST_DEP_TRANS_FLOW' ; --存储过程名
V_TABLE_NAME 	        VARCHAR2(50) :='ST_DEP_TRANS_FLOW';  -- 测试目标表名
...
FULL JOIN
(
select A.TX_DATE,count(1) as ST_NUM from MMAPST.ST_DEP_TRANS_FLOW a where ACCTTYP_ID='D' GROUP BY TX_DATE
) T2
ON T1.Tx_DT=T2.TX_DATE;
...
EXCEPTION
  ...
  P_TEST_WRITE_LOGS(TRANS_NAME,IO_STATUS,IO_ROW,V_START_TIMESTAMP,V_END_TIMESTAMP,VO_SQLERR);
END P_TEST_S_ST_DEP_TRANS_FLOW;