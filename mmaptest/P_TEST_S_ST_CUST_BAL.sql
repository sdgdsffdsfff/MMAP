...
FROM 
(
select A.Tx_DT,COUNT(1) AS DM_NUM    --修改
...
FROM 
(
select A.Tx_DT,SUM(SD_LC_CBAL+CD_LC_CBAL+LN_LC_CBAL+ETR_CBAL+FS_CBAL+LC_CBAL) AS DM_NUM --修改
from   MMAPST.DMMKT_CSLvlCBal A
GROUP BY Tx_DT
) T1
FULL JOIN 
...
FROM 
(
select A.Tx_DT,SUM(SD_FC_CBAL+CD_FC_CBAL+LN_FC_CBAL) AS DM_NUM --修改
from   MMAPST.DMMKT_CSLvlCBal A
GROUP BY Tx_DT
) T1
FULL JOIN 
