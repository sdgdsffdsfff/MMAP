CREATE OR REPLACE PROCEDURE "P_TEST_ST_DM_ACCOUNT_LOAN"
  AS
  
TRANS_NAME VARCHAR2(50) :='P_TEST_ST_DM_ACCOUNT_LOAN' ; --存储过程名

V_TABLE_NAME VARCHAR2(50) :='DM_ACCOUNT_LOAN';  -- 测试目标表名
...
FULL JOIN 
(
SELECT A.TX_DATE,SUM(A.ACCT_Bal) AS DM_NUM  --修改
FROM   MMAPDM.DM_ACCOUNT_LOAN A
GROUP BY A.TX_DATE
)T2
ON T1.TX_DATE=T2.TX_DATE;
