CREATE OR REPLACE PROCEDURE "P_TEST_S_ST_ACCOUNT_DEP"
  AS
  
TRANS_NAME VARCHAR2(50) :='P_TEST_S_ST_ACCOUNT_DEP' ; --存储过程名

V_TABLE_NAME VARCHAR2(50) :='ST_ACCOUNT_DEP';  -- 测试目标表名
...
from   MMAPST.DMMKT_ACLIABI A
GROUP BY Tx_DT
) T1
FULL JOIN 
(
SELECT A.TX_DATE,COUNT(1) AS ST_NUM  --目标修改
FROM   MMAPST.ST_ACCOUNT_DEP A
GROUP BY TX_DATE
)T2
