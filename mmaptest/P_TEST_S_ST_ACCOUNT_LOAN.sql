CREATE OR REPLACE PROCEDURE "P_TEST_S_ST_ACCOUNT_LOAN"
  AS
  
TRANS_NAME VARCHAR2(50) :='P_TEST_S_ST_ACCOUNT_LOAN' ; --存储过程名

V_TABLE_NAME VARCHAR2(50) :='ST_ACCOUNT_LOAN';  -- 测试目标表名
...
from   MMAPST.DMMKT_ACASSET A
GROUP BY Tx_DT
) T1
FULL JOIN 
(
SELECT A.TX_DATE,COUNT(1) AS ST_NUM  --目标修改
FROM   MMAPST.ST_ACCOUNT_LOAN A
GROUP BY TX_DATE
)T2
...
,T1.DM_NUM
,T2.ST_NUM
,NVL(T1.DM_NUM,0)-nvl(T2.ST_NUM,0)
,NULL
FROM 
(
select A.Tx_DT,SUM(A.Acc_Bal) AS DM_NUM --修改
from   MMAPST.DMMKT_ACASSET A
GROUP BY Tx_DT
) T1
FULL JOIN 
(
SELECT A.TX_DATE,SUM(A.ACCT_Bal) AS ST_NUM  --修改
FROM   MMAPST.ST_ACCOUNT_LOAN A
GROUP BY TX_DATE
)T2
ON T1.Tx_DT=T2.TX_DATE;
