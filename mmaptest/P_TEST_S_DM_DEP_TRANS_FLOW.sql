CREATE OR REPLACE PROCEDURE "MMAPTEST"."P_TEST_S_DM_DEP_TRANS_FLOW"

AS
/* 定义变量*/
TRANS_NAME              VARCHAR2(50) :='P_TEST_S_DM_DEP_TRANS_FLOW' ; --存储过程名
V_TABLE_NAME 	        VARCHAR2(50) :='DM_DEP_TRANS_FLOW';  -- 测试目标表名
V_COL 			VARCHAR2(50);     --字段英文名
V_COL_NAME 		VARCHAR2(50);     --字段中文名
V_COL_DES 		VARCHAR2(50);     --字段描述
V_TEST_DES 		VARCHAR2(50);     --测试描述
V_TEST_STAGE 	        VARCHAR2(50) :='S_DM';  -- 测试阶段
...
FROM
(
select A.TX_DT,count(1) as S_NUM from MMAPST.DMMKT_EVDEPSTTRANS a where ACCTTYP_ID='D' GROUP BY TX_DT
) T1
FULL JOIN
(
select A.TX_DATE,count(1) as ST_NUM from MMAPDM.DM_DEP_TRANS_FLOW_HIS a where ACCTTYP_ID='D' GROUP BY TX_DATE
) T2
ON T1.TX_DT=T2.TX_DATE;
...
FROM
(
select A.Tx_DT,SUM(TRANS_AMT1) AS S_NUM --修改
from  MMAPST.DMMKT_EVDEPSTTRANS A
GROUP BY Tx_DT
) T1
FULL JOIN
(
SELECT A.TX_DATE,SUM(TRANS_AMT1) AS ST_NUM  --修改
FROM   MMAPDM.DM_DEP_TRANS_FLOW_HIS A
GROUP BY TX_DATE
)T2
ON T1.Tx_DT=T2.TX_DATE;
...
EXCEPTION
  ...
  P_TEST_WRITE_LOGS(TRANS_NAME,IO_STATUS,IO_ROW,V_START_TIMESTAMP,V_END_TIMESTAMP,VO_SQLERR);
END P_TEST_S_DM_DEP_TRANS_FLOW;