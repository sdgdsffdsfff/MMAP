...
SELECT 
 ...
 ,CASE WHEN TO_CHAR(BIRTH_DT,'MMDD')<TO_CHAR(SYSDATE,'MMDD') 
      THEN FLOOR(MONTHS_BETWEEN(TX_DT,BIRTH_DT)/12)+1 
      ELSE FLOOR(MONTHS_BETWEEN(TX_DT,BIRTH_DT)/12)
  END AS AGE       
 ,BIRTH_DT 
 ,TRIM(GENDER_ID) AS GENDER_ID
 ,TRIM(EDU_ID)  AS EDU_ID
 ,TRIM(MARITALSTS_ID)  AS MARITALSTS_ID
 ,TRIM(CIFCOMP_NM)  AS CIFCOMP_NM
 ,TRIM(INDTYP_ID)  AS INDTYP_ID
 ,TRIM(OCCP_ID)  AS OCCP_ID
 ,TRIM(JOBDES_ID)  AS JOBDES_ID
 ,TRIM(PROFTITL_ID)  AS PROFTITL_ID
 ,NULL AS INCOME  
 ,NULL AS INCOME_RANGE 
 ,TRIM(RESICNTRY_ID)  AS RESICNTRY_ID
 ,TRIM(CITIZEN_ID)  AS CITIZEN_ID
 ,TRIM(RACETYP_ID)  AS RACETYP_ID
 ,NULL AS IDTYPE_CODE  
 ,NULL AS IDCODE
 ...
 ,CIFOPEN_DT
 ,CIFCLOSE_DT
 ,TRIM(CIF_LVL) AS CIF_LVL
 ,TRIM(RISK_LVL) AS RISK_LVL
